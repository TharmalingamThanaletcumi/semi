...
//      beforeSend: function(){
//          $("#loading").dialog('open').html("<p>Patientez svp...</p>");
//       }, 
     error:function(msg){ 
       alert( "Error ! " + msg);       
     },
     success:function(data){
       ...
       // terminer par le couple "nombreInscriptions;0" (zéro, une valeur quelconque pour assurer le couple final)
...
<thead><tr><?php if (! $sessionCloturee)  :?><th class="dataTableHeader">Je m'inscris</th><?php endif;?><th class="dataTableHeader">Disponibilités</th><th class="dataTableHeader">Type</th>
...
<?php foreach ($seances as $seancesHoraire) :  ?>
  ...
  <?php foreach ($seancesHoraire as $seance) :  ?>
   ...
   <td><?php print( (($seance['type']=='atelier') ? ('('.$seance["numRelatif"] . ') ') : '') . $seance['libelle']) ?> </td>
