...
class PdoSeminaire{
	...
	static $DEST_MAIL_CREATION_COMPTE = "dest@example.com";
	static $BCC_MAIL_CREATION_COMPTE = "BCC:bcc@example.com";
	static $RETURN_PATH = "returnpath@example.com";
	...
	/* 
	 * mysql> create user "seminaire"@"localhost";
	 * mysql> GRANT ALL PRIVILEGES ON seminaire.* TO 'seminaire'@'localhost' IDENTIFIED BY '67vHVdpeWKGvqc4e' WITH GRANT OPTION; 
	 * 
	 */
	...
	public function getLesPlagesAtelier($idSem){
		$tab = array();
		try {
			$sql = "select distinct(dateHeureDebut) from seance where seance.type='atelier' and seance.idSeminaire = :IDS order by dateHeureDebut ASC";
			$stmt = self::$monPdo->prepare($sql);
			$stmt->bindParam(':IDS', $idSem);
			$stmt->execute();
			$tab = $stmt->fetchAll();
		} catch (Exception $e) {
			return FALSE;
	...
	public function getLibAtelier($dateDebutAtelier, $idParticipant, $idSem){
		$res = null;
		try {
		 $sql = "select dateHeureDebut, libelle from inscription, seance where seance.dateHeureDebut=:DateDebAtelier and inscription.idSeance = seance.id and seance.idSeminaire = :IDS and idParticipant=:IDP";
		//$sql = "select distinct(dateHeureDebut) from seance where seance.type='atelier' and seance.idSeminaire = :IDS order by dateHeureDebut ASC";
			$stmt = self::$monPdo->prepare($sql);
			$stmt->bindParam(':IDS', $idSem);
			$stmt->bindParam(':DateDebAtelier', $dateDebutAtelier);
			$stmt->bindParam(':IDP', $idParticipant);
			$stmt->execute();
	...
	/**
	 * Recupere tous les inscrits a une seance d'un seminaire
	 * @param $atelier numero de l'atelier concerne
	 * @param $seminaire numero du seminaire concerne
	 */
...
//			$sql = "SELECT participant.nom, participant.prenom, participant.mail, participant.titre, academie.nom acad, participer.priseEnCharge FROM participant, inscription, academie, participer WHERE participant.id = inscription.idParticipant AND idSeance = :IDS AND academie.id = participant.idAcademie AND participer.idParticipant = participant.id AND participer.idSeminaire = :IDSE";
			$sql="SELECT participant.nom, participant.prenom, participant.mail, participant.titre, academie.nom acad FROM participant, inscription, academie, seance WHERE participant.id = inscription.idParticipant AND inscription.idSeance = :IDS AND inscription.idSeance=seance.id AND academie.id = participant.idAcademie AND seance.idSeminaire = :IDSE  ORDER BY participant.nom ASC";
			$stmt = self::$monPdo->prepare($sql);
	...
	/**
	 * Récupere tous les inscrits a un seminaire
	 * @param $seminaire numero du seminaire concerne
	 */
	...
	public function getNbInscritsSeminaire($idSeminaire){
		...
		try {
			...
			$sql = "SELECT count(distinct(idParticipant)) AS nombre FROM inscription, seance  WHERE inscription.idSeance= seance.id and seance.idSeminaire = :idS";
			$stmt = self::$monPdo->prepare($sql);
			$stmt->bindParam(':idS', $idSeminaire);
			$stmt->execute();
			$resu = $stmt->fetchObject();
	...
	/**
	 * Retourne un tableau
	 *  $okUser[0] = $user object, $okUser[1] = boolean cle sem ok
